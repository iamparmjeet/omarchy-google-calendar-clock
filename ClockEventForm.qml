...
Column {
  ...
  function openForEdit(ev) {
    ...
    // The sync schema exposes all-day `end` as an inclusive display date;
    // convert to Google's exclusive date only in sync/mutate.py.
    var endDay = form._dayOf(ev.end)
